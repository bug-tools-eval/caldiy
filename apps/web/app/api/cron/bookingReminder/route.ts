...
async function postHandler(request: NextRequest) {
  ...
  for (const interval of reminderIntervalMinutes) {
    ...
    // Build a Set of already-reminded booking ids once instead of running
    // `reminders.some(...)` per booking (O(B·R) → O(B + R)).
    const remindedBookingIds = new Set(reminders.map((r) => r.referenceId));
    for (const booking of bookingsToRemind.filter((b) => !remindedBookingIds.has(b.id))) {
      const { user } = booking;
